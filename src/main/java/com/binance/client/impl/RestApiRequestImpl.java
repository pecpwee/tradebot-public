...
class RestApiRequestImpl {
    ...
    private Request createRequestWithSignature(String url, String address, UrlParamsBuilder builder) {
        ...
        String requestUrl = url + address;
        new com.binance.client.impl.ApiSignature().createSignature(apiKey, secretKey, builder);
        if (builder.hasPostParam()) {
    ...
    RestApiRequest<CountDownCancelResponse> cancelAllOrder(String symbol, Long countdownTime, Long timestamp) {
        RestApiRequest<CountDownCancelResponse> request = new RestApiRequest<>();
        UrlParamsBuilder builder = UrlParamsBuilder.build()
                .putToUrl("symbol", symbol)
                .putToUrl("countdownTime", countdownTime)
                .putToUrl("timestamp", timestamp);

        request.request = createRequestByGet("/fapi/v1/countdownCancelAll", builder);

        request.jsonParser = (jsonWrapper -> {
            CountDownCancelResponse result = new CountDownCancelResponse();
            result.setSymbol(jsonWrapper.getString("symbol"));
            result.setCountdownTime(jsonWrapper.getString("countdownTime"));
            return result;
        });
        return request;
    ...
    RestApiRequest<List<Candlestick>> getCandlestick(String symbol, CandlestickInterval interval, Long startTime,
        ...
        request.jsonParser = (jsonWrapper -> {
            ...
            dataArray.forEachAsArray((item) -> {
                ...
                element.setNumberOfTrades(item.getLongAt(8));
                element.setTakerBuyBaseAssetVolume(item.getStringAt(9));
                element.setTakerBuyQuoteAssetVolume(item.getStringAt(10));
                // TODO: 2020/10/19 what ?
                element.setTakerBuyQuoteAssetVolume(item.getStringAt(11));
                result.add(element);
            });
    ...
    RestApiRequest<Order> postOrder(String symbol, OrderSide side, PositionSide positionSide, OrderType orderType,
        ...
        UrlParamsBuilder builder = UrlParamsBuilder.build()
                ...
                .putToUrl("closePosition", closePosition ? "true" : "false");
    ...
    RestApiRequest<Order> postTestOrder(String symbol, OrderSide side, PositionSide positionSide, OrderType orderType,
                                        TimeInForce timeInForce, String quantity, String price, String reduceOnly,
        ...
        request.request = createRequestByPostWithSignature("/fapi/v1/order/test", builder);
    ...
    RestApiRequest<CountDownCancelResponse> cancelAllOpenOrder(String symbol, Long countdownTime, Long timestamp) {
        RestApiRequest<CountDownCancelResponse> request = new RestApiRequest<>();
        UrlParamsBuilder builder = UrlParamsBuilder.build()
                .putToUrl("symbol", symbol)
                .putToUrl("countdownTime", countdownTime)
                .putToUrl("timestamp", timestamp);
        request.request = createRequestByPostWithSignature("/fapi/v1/s/countdownCancelAll", builder);

        request.jsonParser = (jsonWrapper -> {
            CountDownCancelResponse countDownCancelResponse = new CountDownCancelResponse();
            countDownCancelResponse.setSymbol(jsonWrapper.getString("symbol"));
            countDownCancelResponse.setCountdownTime(jsonWrapper.getString("timestamp"));
            return countDownCancelResponse;
        });
        return request;
    }
